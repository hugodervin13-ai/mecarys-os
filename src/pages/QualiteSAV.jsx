    ...
    <div>
      ...
      <div className="flex gap-4 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === 'reviews' ? 'bg-[#5a2d82] text-white' : 'text-[#a0aec0] hover:text-white'}`}
        >
          Avis negatifs
        </button>
        <button
          onClick={() => setTab('returns')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === 'returns' ? 'bg-[#5a2d82] text-white' : 'text-[#a0aec0] hover:text-white'}`}
        >
          Retours
        </button>
      </div>
      ...
      {tab === 'reviews' && (
        <div className="space-y-4">
          {mockReviews.map((review) => (
            <div key={review.id} className="bg-[#1a1f2e] rounded-xl p-5 border border-[#2d3748]">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                  <div className="flex">
                    {[1,2,3,4,5].map(s => (
                      <span key={s} className={`text-sm ${s <= review.rating ? 'text-[#f59e0b]' : 'text-[#2d3748]'}`}>★</span>
                    ))}
                  </div>
                  <span className="text-[#a0aec0] text-xs">{review.asin}</span>
                </div>
              ...
              <p className="text-[#a0aec0] text-xs">{review.date}</p>
            </div>
          ))}
        </div>
      )}
          ...
          <table className="w-full">
            <thead className="bg-[#0f1419]">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-[#a0aec0] text-sm font-medium">Raison</th>
            ...
            <tbody>
              {mockReturns.map((r) => (
                <tr key={r.id} className="border-t border-[#2d3748] hover:bg-[#2d3748]/30">
                  <td className="px-6 py-4 text-white text-sm font-mono">{r.asin}</td>
                  <td className="px-6 py-4 text-white text-sm">{r.reason}</td>
                  <td className="px-6 py-4 text-white text-sm">{r.quantity}</td>
                  <td className="px-6 py-4 text-[#ef4444] text-sm">{r.cost} &euro;</td>
                  <td className="px-6 py-4 text-[#a0aec0] text-sm">{r.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
