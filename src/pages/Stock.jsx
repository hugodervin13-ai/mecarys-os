...
import { box, tableHeader } from '../lib/theme'
...
export default function Stock() {
  ...
  const saveAlerte = async (id) => {
    ...
    const ok = await mutate(() => updateProduct(id, { stock_alerte: Number(newAlerte) }), 'products')
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#9ca3af', marginTop: 3 }}>Anticipez les ruptures et planifiez vos réapprovisionnements</p>
        </div>
      </div>
      ...
      {urgentCount > 0 && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '14px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: '#ef4444', marginTop: 2 }}>Commandez maintenant pour éviter une perte de classement BSR</div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 22 }}>
        <KpiCard label="Stock total FBA" value={formatNumber(totalStock)} icon="🏭" color="#6366f1" sub="unités en entrepôt Amazon" />
        <KpiCard label="Jours de stock (moy.)" value={avgDays ? `${avgDays}j` : '-'} icon="📅" color="#3b82f6" sub={avgDays && avgDays < 30 ? '⚠️ Niveau bas en moyenne' : 'Niveau satisfaisant'} />
        <KpiCard label="Urgents (< 7 jours)" value={urgentCount} icon="🚨" color="#ef4444" sub={urgentCount > 0 ? 'Commander immédiatement' : 'Aucune urgence'} />
        <KpiCard label="Stock faible (7-14j)" value={lowCount} icon="⚠️" color="#f59e0b" sub="À surveiller de près" />
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ padding: '7px 12px', background: '#fafaf8', border: '1px solid #e8e8e3', borderRadius: 8, color: '#6b7280', fontSize: 12, cursor: 'pointer' }}>
          ...
          <option value="velocity">Trier : Vélocité</option>
      ...
      <div style={{ ...box, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#fafaf8' }}>
              {['Produit', 'Stock FBA', 'Jours restants', 'Seuil alerte', 'Vélocité / jour', 'Commander (60j)', 'Statut'].map(h => (
                  ...
                  <td style={{ padding: '13px 16px' }}>
                    {p.restock ? (
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: '#9ca3af' }}>unités</div>
                      </div>
                    ) : <span style={{ fontSize: 12, color: '#9ca3af' }}>-</span>}
                  </td>
                  <td style={{ padding: '13px 16px' }}>
        ...
        {filtered.length === 0 && (
          <EmptyState icon="🏭" title="Aucun produit" subtitle="Ajoutez des produits avec leur stock FBA pour les suivre ici" />
        )}
      </div>
