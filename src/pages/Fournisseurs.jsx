...
import { listSuppliers, createSupplier, updateSupplier, removeSupplier } from '../lib/suppliersRepo'
import { loadOrders } from '../lib/ordersRepo'
...
const SORT_OPTIONS = [
  ...
  { value: 'orders', label: 'Nb commandes' },
]

const emptyForm = { name: '', country: '', contact_email: '', phone: '', website: '', notes: '', status: 'active' }
...
export default function Fournisseurs() {
  ...
  const openEdit = (s) => {
    setForm({
      name: s.name || '', country: s.country || '', contact_email: s.contact_email || '',
      phone: s.phone || '', website: s.website || '', notes: s.notes || '', status: s.status || 'active',
    })
    setEditingId(s.id)
    setShowForm(true)
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (editingId) {
      const next = await updateSupplier(uid, editingId, form)
      setSuppliers(next)
  ...
  const filtered = useMemo(() => {
    ...
    let list = suppliers
      ...
      .filter(s => !q || [s.name, s.country, s.contact_email, s.website].some(v => (v || '').toLowerCase().includes(q)))

    if (sortBy === 'name') list = [...list].sort((a, b) => (a.name || '').localeCompare(b.name || ''))
    else if (sortBy === 'country') list = [...list].sort((a, b) => (a.country || '').localeCompare(b.country || ''))
    else if (sortBy === 'orders') list = [...list].sort((a, b) => (orderCounts[b.name] || 0) - (orderCounts[a.name] || 0))
    // 'recent' = default order from repo

    return list
  }, [suppliers, filterStatus, search, sortBy, orderCounts])
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 22 }}>
        ...
        <KpiCard label="Commandes" value={totalOrders} icon="📋" color={colors.warning} sub="Total passées" />
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <input style={{ ...inp, maxWidth: 240 }} placeholder="🔍 Nom, pays, email…" value={search} onChange={e => setSearch(e.target.value)} />
        <select style={{ ...inp, maxWidth: 160 }} value={sortBy} onChange={e => setSortBy(e.target.value)}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 14 }}>
        {filtered.map(s => {
          const cmdCount = orderCounts[s.name] || 0
          return (
            <div key={s.id} style={{ ...box, padding: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  ...
                  {cmdCount > 0 && (
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 20, background: `${colors.primary}12`, color: colors.primary }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 12, borderTop: '1px solid #f0f0eb' }}>
                ...
                {s.website && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 14 }}>🌐</span>
                    <a href={s.website.startsWith('http') ? s.website : `https://${s.website}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: colors.info, textDecoration: 'none', fontWeight: 500 }}>{s.website}</a>
                  </div>
                )}
                {s.notes && (
      ...
      <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={editingId ? 'Modifier le fournisseur' : 'Nouveau fournisseur'}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <input style={inp} type="text" placeholder="www.supplier.com" value={form.website} onChange={e => setForm({ ...form, website: e.target.value })} />
          </div>
