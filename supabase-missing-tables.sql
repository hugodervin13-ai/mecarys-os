...
-- 4. (Optionnel) Bucket Storage pour les fichiers
-- Aller dans Supabase > Storage > New bucket > "documents" (private)
-- Policy INSERT : (bucket_id = 'documents') AND (auth.uid()::text = (storage.foldername(name))[1])
-- Policy SELECT : (bucket_id = 'documents') AND (auth.uid()::text = (storage.foldername(name))[1])
